...
def generate_agent_token(length: int = 64) -> str:
...
async def get_current_user(
    ...
    4. We return the User object (or raise 401)
...
def verify_agent_token(agent_token: str, db: Session) -> Optional[models.Agent]:

    agent = db.query(models.Agent).filter(models.Agent.token == agent_token).first()
